...
----
compile 'io.vertx:vertx-health-check:3.5.0-SNAPSHOT'
----
...
healthCheckHandler1 = VertxHealthChecks::HealthCheckHandler.create(vertx)
healthCheckHandler2 = VertxHealthChecks::HealthCheckHandler.create_with_health_checks(VertxHealthChecks::HealthChecks.create(vertx))

router = VertxWeb::Router.router(vertx)
# Populate the router with routes...
# Register the health check handler
router.get("/health*").handler(&healthCheckHandler1.method(:handle))
# Or
router.get("/ping*").handler(&healthCheckHandler2.method(:handle))

----

Procedure registration can be directly made on the `link:../../yardoc/VertxHealthChecks/HealthCheckHandler.html[HealthCheckHandler]`
...
----
require 'vertx-health-checks/health_check_handler'
healthCheckHandler = VertxHealthChecks::HealthCheckHandler.create(vertx)
...
router.get("/health").handler(&healthCheckHandler.method(:handle))
...
`link:../../yardoc/VertxHealthChecks/HealthCheckHandler.html[HealthCheckHandler]`.
